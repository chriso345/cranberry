...
class TestParseContext:
    def test_command_stored(self):
        ctx = ParseContext(command="cmd", globals_obj=None)
        assert ctx.command == "cmd"

    def test_command_none(self):
        ctx = ParseContext(command=None, globals_obj=None)
        assert ctx.command is None

    def test_globals_become_attributes(self):
        ctx = ParseContext(
            command=None, globals_obj=SimpleNamespace(flag=True, count=5)
        )
        assert ctx.flag is True
        assert ctx.count == 5

    def test_globals_object_is_typed_and_accessible(self):
        globals_obj = SimpleNamespace(flag=True)
        ctx = ParseContext(command=None, globals_obj=globals_obj)
        assert ctx.globals is globals_obj
        assert ctx.globals.flag is True
    ...
    def test_repr_shows_command(self):
        ctx = ParseContext(command="my-cmd", globals_obj=None)
        assert "my-cmd" in repr(ctx)

    def test_repr_shows_globals(self):
        ctx = ParseContext(command=None, globals_obj=SimpleNamespace(key="val"))
